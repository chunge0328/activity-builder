...
function _flushFile(data, configStorage) {
	let appFile = path.join(config.ACTIVITY_BASE_DIR, './App.vue');
	let map = {};
	let comps = [];
...
`<script>
  ...
  `;
	comps.forEach(function(name) {
		appTpl += 
  `import ${name} from 'components/business/${name}/index.vue';
  `;
	});
	appTpl += 
  `vue.use({
...
function _init(callback) {
    compiler = webpack({
	    ...
	    entry: {
            index: ["webpack/hot/dev-server", `webpack-dev-server/client?${config.INTERNAL_SERVER_HOST}`, "./entry-client"],
