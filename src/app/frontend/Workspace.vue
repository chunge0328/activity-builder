    ...
    export default {
		...
		methods: {
            inspectNode: function() {
                ...
                let self = this;
                let allInstances = backend.getAllInstances(doc);
                this.clearResizeState();
                doc.documentElement.onmousemove = function(event) {
                    if(backend.isFixedNode()) return;
                    var got = self._isInNode(event.clientX, event.clientY, allInstances);
                    if(got) {
                        backend.setHighlightColor('rgba(104, 182, 255, 0.35)');
                        backend.highlight(got);
                        self.inspectedContext = self.$refs.design.contentWindow;
                        self.inspectedNode = backend.getInstance(got);
                    } else {
                        backend.unHighlight();
                    }
                }
            ...
            _isInNode: function(x, y, all) {
                function walk(instance) {
                    for(var i = 0; i < instance.children.length; i++) {
                        ...
                        c.rect = backend.getInstanceRect(c);
            ...
            makeActivity: function() {
                backend.clear();
                backend.flushFile(this.selectedComponents, this.configStorage);
            },
            ...
            save: function() {
                ...
                remote.capturePage(this._transformRect(this.$refs.design.getBoundingClientRect()), function(imageData) {
	                ...
	                if(!self.tpl._id) {
		                backend.getDB().insert(data, function (err, newDoc) {
		                	if(err) return;
                            self.$store.commit('setActiveTpl', newDoc);
	                    });
		            } else {
		            	backend.getDB().update({_id: self.tpl._id}, data, {returnUpdatedDocs: true}, function(err, numReplaced, doc) {
                            if(err) return;
                            if(numReplaced) {
            ...
            resize: function() {
                ...
                doc.documentElement.onmousemove = function(event) {
                    ...
                    var got = self._isInNode(event.clientX, event.clientY, allInstances);
                    if(got) {                      
                        backend.setHighlightColor('rgba(24, 255, 255, 0.35)');
                        backend.highlight(got, function() {
                            self.isResizingNode = true;
                            new Resizing(got, self.configStorage[util.locate(backend.getInstance(got))]).activate();
                            backend.unHighlight();
                        });
                    } 
                }
            ...
            clearResizeState: function() {
                ...
                if(obj) {
                    var location = util.locate(backend.getInstance(obj._node));
